...
void mtxf_mul_vec3s(Mat4 mtx, Vec3s b) {
    ...
    shz_vec3_t out = shz_xmtrx_trans_vec3((shz_vec3_t){.x = x, .y = y, .z = z});
    b[0] = out.x;
    b[1] = out.y;
    b[2] = out.z;
...
void get_pos_from_transform_mtx(Vec3f dest, Mat4 objMtx, Mat4 camMtx) {
    shz_vec3_t camXYZ = shz_vec3_dot3(shz_vec3_deref(camMtx[3]),
        shz_vec3_deref(camMtx[0]),
        shz_vec3_deref(camMtx[1]),
        shz_vec3_deref(camMtx[2]));

    shz_vec3_t destXYZ = shz_vec3_dot3(shz_vec3_deref(objMtx[3]),
        shz_vec3_deref(camMtx[0]),
        shz_vec3_deref(camMtx[1]),
        shz_vec3_deref(camMtx[2]));

    dest[0] = destXYZ.x - camXYZ.x; /* shz_dot8f(objMtx[3][0], objMtx[3][1], objMtx[3][2], camXYZ.x,
                        camMtx[0][0], camMtx[0][1], camMtx[0][2], -1.0f); */
    dest[1] = destXYZ.y - camXYZ.y; /* shz_dot8f(objMtx[3][0], objMtx[3][1], objMtx[3][2], camXYZ.y,
                        camMtx[1][0], camMtx[1][1], camMtx[1][2], -1.0f); */
    dest[2] = destXYZ.z - camXYZ.z; /* shz_dot8f(objMtx[3][0], objMtx[3][1], objMtx[3][2], camXYZ.z,
...
void vec3f_get_dist_and_angle(Vec3f from, Vec3f to, f32 *dist, s16 *pitch, s16 *yaw) {
    ...
    *dist = shz_sqrtf_fsrra(shz_mag_sqr3f(x, y, z)); // (x * x + y * y + z * z);
