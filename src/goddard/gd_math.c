...
void gd_mat4f_lookat(Mat4f *mtx, f32 xFrom, f32 yFrom, f32 zFrom, f32 xTo, f32 yTo, f32 zTo,
    ...
    invLength = -shz_vec3_magnitude_inv(shz_vec3_deref(&d));
    d.z *= invLength;
    d.y *= invLength;
    d.x *= invLength;
    ...
    invLength = shz_vec3_magnitude_inv(shz_vec3_deref(&colX));
...
void gd_create_origin_lookat(Mat4f *mtx, struct GdVec3f *vec, f32 roll) {
    ...
    gd_set_identity_mat4(mtx);
    if (hMag != 0.0f) {
        invertedHMag = shz_fast_invf(hMag);
...
void gd_absrot_mat4(Mat4f *mtx, s32 axisnum, f32 ang) {
    ...
    gd_create_rot_mat_angular(&rMat, &rot, ang * 0.5f);
    gd_mult_mat4f((const Mat4f *)mtx, (const Mat4f *)&rMat, mtx);
}


f32 gd_vec3f_magnitude(struct GdVec3f *vec) {
    return gd_sqrt_f(shz_mag_sqr3f(vec->x, vec->y, vec->z));
}
...
s32 gd_normalize_vec3f(struct GdVec3f *vec) {
    ...
    mag = shz_mag_sqr3f(vec->x, vec->y, vec->z);
...
void gd_inverse_mat4f(Mat4f *src, Mat4f *dst) {
    ...
    f32 invdet = shz_fast_invf(determinant);
...
f32 gd_mat4f_det(Mat4f *mtx) {
    ...
    det = shz_dot8f(inv.r3.c3, -inv.r2.c3, inv.r1.c3, -inv.r0.c3,
...
f32 gd_3x3_det(f32 r0c0, f32 r0c1, f32 r0c2,
    ...
    det = shz_dot6f(r0c0, -r1c0, r2c0,
                    gd_2x2_det(r1c1, r1c2, r2c1, r2c2),
                    gd_2x2_det(r0c1, r0c2, r2c1, r2c2),
                    gd_2x2_det(r0c1, r0c2, r1c1, r1c2));
...
void gd_rotate_and_translate_vec3f(struct GdVec3f *vec, const Mat4f *mtx) {
    ...
    shz_xmtrx_load_4x4((const shz_matrix_4x4_t *)&temp);
    shz_vec3_t res = shz_xmtrx_trans_vec3((shz_vec3_t) { .x = vec->x, .y = vec->y, .z = vec->z} );
...
void gd_mult_mat4f(const Mat4f *mA, const Mat4f *mB, Mat4f *dst) {
    shz_xmtrx_load_4x4_apply_store((shz_matrix_4x4_t *)dst,
        (const shz_matrix_4x4_t *)mB, (const shz_matrix_4x4_t *)mA);
}
